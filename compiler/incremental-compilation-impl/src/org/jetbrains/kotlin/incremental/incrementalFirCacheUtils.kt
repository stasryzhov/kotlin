...
internal fun collectNewDirtySources(
    ...
    analysisResults.fir.forEach {
        it.accept(object : FirVisitor<Unit, MutableList<MetadataSerializer>>() {
            ...
            override fun visitPropertyAccessor(propertyAccessor: FirPropertyAccessor, data: MutableList<MetadataSerializer>) {
//                    propertyAccessor.
                super.visitPropertyAccessor(propertyAccessor, data)
            }

            override fun visitSimpleFunction(function: FirSimpleFunction, data: MutableList<MetadataSerializer>) {
                data.firstOrNull()?.let {
                    super.visitFunction(function, data)
                    it.bindMethodMetadata(
                        FirMetadataSource.Function(function),
                        Method(function.name.asString(), function.computeJvmDescriptor())
                    )
                }
            }
