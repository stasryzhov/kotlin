...
internal fun ObjCExportFunctionGenerationContext.callAndMaybeRetainAutoreleased(
    ...
    // (implemented as objc_retainAutoreleaseReturnValue and objc_autoreleaseReturnValue).

    // In order for this to work properly, we need the generated machine code to be like:
    // call to function and then call to objcRetainAutoreleasedReturnValue, with nothing between
    // (except for moving return value of the former to argument of the latter, if necessary).
    // objc_autoreleaseReturnValue and others detect this particular pattern.
    //
    ...
    // a redundant jump between the calls when generating the machine code.
    // To workaround this, we generate these two calls as a separate noinline function ("outlined"),
    // and catch the exceptions in the caller of this function.
    // So, no exception handler in "outlined" => no redundant branch => the optimized return works properly.
